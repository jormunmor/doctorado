...
import optaplannersolutionparser.OptaPlannerSolutionParser;
...
public class SolverAndPersistenceFrame extends JFrame {
    ...
    //public static SolutionBusiness solutionBusiness; /// @jorge
    ...
    public SolverAndPersistenceFrame(SolutionBusiness solutionBusiness, SolutionPanel solutionPanel,
            String titlePrefix) {
        super(titlePrefix);
        //this.parser = new OptaPlannerSolutionParser(solutionPath, updatedSystemConfigPath, tasksPath, outPddlPath);
    ...
    public void bestSolutionChanged() {
        ...
        /*
            The following code was added to try to insert JSHOP2 capabilities in
            OptaPlanner. Another file modified was the SolverAndPersistenceFrame.java
            in the package org.optaplanner.examples.common.swingui.
        */
        //File solutionFile = new File(solutionPath);
        // Store the OptaPlanner assignment
        //solutionBusiness.saveSolution(solutionFile);
        // Parse the assignment and generate the JSHOP2 problem
        ...
        try {
            ...
            // Invoke the JSHOP2 process
            Runtime runtime = Runtime.getRuntime();
            Process process = runtime.exec(execString);
            process.waitFor();
            
            // Check if the plan file (planJShop2.txt) was created, and if true, move to tmp
            // and update softScore.
            File planFile = new File(planPath);
            if(planFile.exists() && !planFile.isDirectory()) {
                File newPlan = new File(finalPlanPath);
                planFile.renameTo(newPlan);
                File newSolution = new File(finalSolutionPath);
                solutionFile.renameTo(newSolution);
                
            }
            
        } catch (IOException ex) {
            java.util.logging.Logger.getLogger(SolverAndPersistenceFrame.class.getName()).log(Level.SEVERE, null, ex);
